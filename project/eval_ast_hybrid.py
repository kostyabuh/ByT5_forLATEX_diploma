...
from spoken_math_ast_parser import ast_parse


warnings.filterwarnings("ignore")
logging.set_verbosity_error()

# Используем уже сохранённый лучший base top1 из n-best прогона
INPUT_PATH = Path(r"project\artifacts\nbest_rerank\unused_rest_base_nbest_rerank.parquet")
OUT_PATH = Path(r"project\artifacts\ast_hybrid\unused_rest_eval.parquet")

# Только реально "структурные" семейства AST
WHITELIST = {
    "fraction",
    "log_base",
    "natlog",
...
def main():
    ...
    for i, row in enumerate(df.itertuples(index=False), start=1):
        ast_pred, ast_family, ast_conf = ast_parse(row.input_text)
    ...
    print(
        ...
        f"{pd.Series([normalized_exact_match(p, t) for p, t in zip(out['base_prediction'], out['target_text'])]).mean():.4f}"
    )
    print()
