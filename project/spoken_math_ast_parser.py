...
def symbol_from_token(tok: str) -> str | None:
...
class Parser:
    ...
    def is_factor_start(self, tok: str | None) -> bool:
        if tok is None:
            return False
        if tok in {
            "LPAR", "MINUS", "SQRT", "NATLOG", "LOG", "DERIVBY", "PDERIVBY",
            "LIMITWHEN", "SUMFROM"
        }:
            return True
        if tok in TRIG_TOKENS:
            return True
        return symbol_from_token(tok) is not None
    ...
    def parse_mul(self, stop: set[str]) -> Node:
        ...
        left = self.parse_postfix(stop=mul_stops)
        ...
        while True:
            ...
            if tok == "TIMES" and tok not in stop:
                self.advance()
                right = self.parse_postfix(stop=mul_stops)
    ...
    def parse_postfix(self, stop: set[str]) -> Node:
        postfix_stops = stop | {"INDEX", "POW", "POW2", "POW3", "POW4", "POW5"}
    ...
    def parse_prefix(self, stop: set[str]) -> Node:
        ...
        if tok == "LOG":
            self.advance()
            arg = self.parse_relation(stop={"BASEOF"})
            self.expect("BASEOF")
            base = self.parse_relation(stop=stop)
        ...
        # Prefix fraction form: "дробь ... DIVBY ..."
        if tok == "дробь":
            self.advance()
...
def infer_confidence(node: Node) -> float:
    ...
    if family in {"fraction", "log_base", "natlog", "sqrt"}:
        return 0.99
    if family in {"derivative", "limit", "sum"}:
        return 0.97
    if family == "trig":
        return 0.95
    if family == "power_index":
        return 0.92
    if family == "relation":
        return 0.90
    return 0.85
...
def ast_parse(text: str) -> tuple[str | None, str | None, float]:
    try:
