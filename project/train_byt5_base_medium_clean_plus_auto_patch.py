...
PATCH_PATH = Path(r"project\data\processed\gold_train_patch_auto.parquet")
VAL_PATH = Path(r"project\data\processed\s2l_ru_equations_medium_clean_unused_val_rest.parquet")

BASE_MODEL_DIR = Path(r"project\artifacts\byt5_base_mathbridge_ru_medium_clean\final_model")
ARTIFACT_DIR = Path(r"project\artifacts\byt5_base_mathbridge_ru_medium_clean_plus_auto_patch")
...
def main():
    ...
    patch = load_df(PATCH_PATH)
    val_df = load_df(VAL_PATH)

    patch_rep = pd.concat([patch.copy() for _ in range(PATCH_REPEAT)], ignore_index=True)
    train_df = pd.concat([base_train, patch_rep], ignore_index=True)

    print(f"base train rows:      {len(base_train)}")
    print(f"patch unique rows:    {len(patch)}")
    print(f"patch repeated rows:  {len(patch_rep)}")
