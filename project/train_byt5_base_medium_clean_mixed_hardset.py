...
BASE_TRAIN_PATH = Path(r"project\data\processed\s2l_ru_equations_medium_clean_train.parquet")
HARDSET_PATH = Path(r"project\data\processed\ru_hardset_preview_pairs.parquet")
VAL_PATH = Path(r"project\data\processed\s2l_ru_equations_medium_clean_unused_val.parquet")

BASE_MODEL_DIR = Path(r"project\artifacts\byt5_base_mathbridge_ru_medium_clean\final_model")
ARTIFACT_DIR = Path(r"project\artifacts\byt5_base_mathbridge_ru_medium_clean_mixed_hardset")
...
def main():
    ...
    base_train = load_df(BASE_TRAIN_PATH)
    hardset = load_df(HARDSET_PATH)
    val_df = load_df(VAL_PATH)

    hardset_rep = pd.concat([hardset.copy() for _ in range(HARDSET_REPEAT)], ignore_index=True)
    train_df = pd.concat([base_train, hardset_rep], ignore_index=True)

    print(f"base train rows:        {len(base_train)}")
    print(f"hardset unique rows:    {len(hardset)}")
    print(f"hardset repeated rows:  {len(hardset_rep)}")
    print(f"final train rows:       {len(train_df)}")
    print(f"unused val rows:        {len(val_df)}")
