...
OUT_PATH = Path(r"project\artifacts\holdout_compare\unused_rest_base_vs_auto_patch.parquet")
...
def main():
    ...
    patch_part = run_model(PATCHED_MODEL_DIR, df, "patched")

    result = pd.concat([df, base_part, patch_part], axis=1)
    ...
    print()
    print(f"patched raw exact:        {result['patched_raw_exact_match'].mean():.4f}")
    print(f"patched normalized exact: {result['patched_normalized_exact_match'].mean():.4f}")
    print()

    improved = result[
        (result["base_normalized_exact_match"] == 0) &
        (result["patched_normalized_exact_match"] == 1)
    ]
    print(f"patched improvements over base: {len(improved)}")
    print()
    if len(improved):
        print(
            improved[
