...
TRAIN_PATH = Path(r"project\data\processed\ru_hardset_preview_pairs.parquet")
...
ARTIFACT_DIR = Path(r"project\artifacts\byt5_base_mathbridge_ru_medium_clean_rescue")
...
def main():
    ...
    # многократно повторяем hardset, иначе он слишком маленький
    train_df = pd.concat([train_df.copy() for _ in range(HARDSET_REPEAT)], ignore_index=True)

    print(f"hardset rows after repeat: {len(train_df)}")
